...
read_headset = True
window = 3 if len(sys.argv) < 2 else int(sys.argv[1])
    ...
    if state == 0:
        ...
        if ready == 1:
            for i in channelList:
                ...
                if result == 0:    #EDK_OK
                    engagement.append(low_betaValue.value / (alphaValue.value + thetaValue.value))
                if (time.time() - start_time >= window) and read_headset:
                    e = int(round(reduce(lambda x, y: x + y, engagement) / len(engagement) * 20))
